...
const LOCALIZED_HOROSCOPE_OVERVIEWS = {
...
function generateHoroscope(apiTexts) {
  ...
  const horoscopes = HOROSCOPE_TEMPLATES.map((tpl, index) => {
    ...
    const apiText = apiTexts?.[SIGN_API_KEYS[index]] || '';
    const overall = /[\u3400-\u9fff]/.test(apiText)
      ? apiText
      : (LOCALIZED_HOROSCOPE_OVERVIEWS[tpl.signEn] || replaceTrait(pick(FORTUNE_TEXTS.overall, index, 0)));
